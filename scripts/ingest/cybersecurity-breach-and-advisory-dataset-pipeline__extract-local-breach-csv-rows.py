...
Output: JSON-compatible result written to data/raw/cybersecurity-breach-and-advisory-dataset-pipeline/extract-local-breach-csv-rows.json when --output is supplied.
...
NODE_NAME = 'Extract local breach CSV rows'
NODE_TYPE = 'n8n-nodes-base.extractFromFile'
CLASSIFICATION = 'ingest'
DEFAULT_OUTPUT = 'data/raw/cybersecurity-breach-and-advisory-dataset-pipeline/extract-local-breach-csv-rows.json'
...
def extract_local_breach_csv_rows(payload: Any) -> dict[str, Any]:
    """Capture a local source snapshot or approval-required external source handoff for this converted n8n source node.
...
if __name__ == "__main__":
    ...
    write_json(extract_local_breach_csv_rows(payload["data"]), payload["output"])
