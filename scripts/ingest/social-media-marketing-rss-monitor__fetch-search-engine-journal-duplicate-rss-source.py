...
Output: JSON-compatible result written to data/raw/social-media-marketing-rss-monitor/fetch-search-engine-journal-duplicate-rss-source.json when --output is supplied.
...
NODE_NAME = 'Fetch Search Engine Journal duplicate RSS source'
NODE_TYPE = 'n8n-nodes-base.rssFeedRead'
CLASSIFICATION = 'ingest'
DEFAULT_OUTPUT = 'data/raw/social-media-marketing-rss-monitor/fetch-search-engine-journal-duplicate-rss-source.json'
...
def fetch_search_engine_journal_duplicate_rss_source(payload: Any) -> dict[str, Any]:
    """Capture a local source snapshot or approval-required external source handoff for this converted n8n source node.
...
if __name__ == "__main__":
    ...
    write_json(fetch_search_engine_journal_duplicate_rss_source(payload["data"]), payload["output"])
