...
Output: JSON-compatible result written to data/raw/brand-reputation-news-intelligence-pipeline/fetch-hugging-face-text-dataset-rows.json when --output is supplied.
...
NODE_NAME = 'Fetch Hugging Face text dataset rows'
NODE_TYPE = 'n8n-nodes-base.httpRequest'
CLASSIFICATION = 'ingest'
DEFAULT_OUTPUT = 'data/raw/brand-reputation-news-intelligence-pipeline/fetch-hugging-face-text-dataset-rows.json'
...
def fetch_hugging_face_text_dataset_rows(payload: Any) -> dict[str, Any]:
    """Capture a local source snapshot or approval-required external source handoff for this converted n8n source node.
...
if __name__ == "__main__":
    ...
    write_json(fetch_hugging_face_text_dataset_rows(payload["data"]), payload["output"])
