...
Output: JSON-compatible result written to data/raw/cybersecurity-breach-and-advisory-dataset-pipeline/read-local-breach-csv-source.json when --output is supplied.
...
NODE_NAME = 'Read local breach CSV source'
NODE_TYPE = 'n8n-nodes-base.readWriteFile'
CLASSIFICATION = 'ingest'
DEFAULT_OUTPUT = 'data/raw/cybersecurity-breach-and-advisory-dataset-pipeline/read-local-breach-csv-source.json'
...
def read_local_breach_csv_source(payload: Any) -> dict[str, Any]:
    """Capture a local source snapshot or approval-required external source handoff for this converted n8n source node.
...
if __name__ == "__main__":
    ...
    write_json(read_local_breach_csv_source(payload["data"]), payload["output"])
