...
| Fetch NewsAPI AWS data-center hiring articles | `n8n-nodes-base.httpRequest` | `https://newsapi.org/v2/everything?q=data+center+hiring+AWS&pageSize=100&apiKey=[REDACTED_USE_ENV]` | Credential was embedded in the original n8n URL; replace with environment variable and record `[TO DO] credential migration` before live use. |
...
| Fetch NewsAPI AWS data-center hiring articles | Source payload | `https://newsapi.org/v2/everything?q=data+center+hiring+AWS&pageSize=100&apiKey=[REDACTED_USE_ENV]` | Yes |

## Phase Gates

1. Source identity gate: verify that `pantry/bykovdenis_454895_41780985_Bykov_Denis_A3_Workflow.json` is the intended workflow and that this recipe title describes the actual work. Test: `test -f "pantry/bykovdenis_454895_41780985_Bykov_Denis_A3_Workflow.json"` and compare the Source Inventory against the original n8n JSON. Human capacity: [PF], [TO].
...
5. Cleanup rule gate: GIGO outputs expose record count, rejects, duplicates, missing fields, and required-field assumptions. Test: inspect `data/verified/data-center-hiring-signal-monitor/`; if critical fields are undefined, add `[TO DO] define required fields` and stop. Human capacity: [IJ].
...
1. Step name: Verify provenance and source intent. Labor: AI plus Human. Script called: none. Input: `pantry/bykovdenis_454895_41780985_Bykov_Denis_A3_Workflow.json`. Output: provenance and title check. Where output goes: `logs/`. Human check: confirm this recipe is named for the work it does and not for a submitter or assignment label.
2. Step name: Fetch BLS labor time-series data. Labor: AI with Human gate. Script called: `scripts/ingest/data-center-hiring-signal-monitor__fetch-bls-labor-time-series-data.py`. Input: prior step output or approved local sample. Output: ingest result JSON. Where output goes: `data/raw/`. Human check: confirm source URL/path, allowed live access, credential handling, and sample-vs-live boundary.
3. Step name: Fetch NewsAPI AWS data-center hiring articles. Labor: AI with Human gate. Script called: `scripts/ingest/data-center-hiring-signal-monitor__fetch-newsapi-aws-data-center-hiring-articles.py`. Input: prior step output or approved local sample. Output: ingest result JSON. Where output goes: `data/raw/`. Human check: confirm source URL/path, allowed live access, credential handling, and sample-vs-live boundary.
4. Step name: Parse local data-center job-market sample records. Labor: AI with Human gate. Script called: `scripts/gigo/data-center-hiring-signal-monitor__parse-local-data-center-job-market-sample-records.py`. Input: prior step output or approved local sample. Output: gigo result JSON. Where output goes: `data/verified/`. Human check: inspect cleanup assumptions, rejects, duplicates, missing fields, and critical-field definitions.
5. Step name: Merge, clean, and deduplicate hiring signals. Labor: AI with Human gate. Script called: `scripts/gigo/data-center-hiring-signal-monitor__merge-clean-and-deduplicate-hiring-signals.py`. Input: prior step output or approved local sample. Output: gigo result JSON. Where output goes: `data/verified/`. Human check: inspect cleanup assumptions, rejects, duplicates, missing fields, and critical-field definitions.
6. Step name: Prepare data-center hiring signal export. Labor: AI with Human gate. Script called: `scripts/tools/data-center-hiring-signal-monitor__prepare-data-center-hiring-signal-export.py`. Input: prior step output or approved local sample. Output: tool result JSON. Where output goes: `logs/`. Human check: approve output contract and ensure no live export/write/send occurs without sign-off.
7. Step name: Produce human report. Labor: AI with Human review. Script called: none; conductor fills `reports/templates/data-center-hiring-signal-monitor.md`. Input: run log and verified outputs. Output: decision report. Where output goes: `reports/generated/`. Human check: read sources, findings, anomalies, `[TO DO]` gaps, and decisions before treating findings as evidence.
...
The agent output goes to `logs/data-center-hiring-signal-monitor-[DATE].json` and contains `workflow`, `source_json`, `source_inventory`, `mode`, `steps_completed`, `records_seen`, `rejects`, `duplicates`, `handoffs`, `flags`, `todo_items`, `stop_conditions`, and `generated_at`.

### Human report

The human report goes to `reports/generated/data-center-hiring-signal-monitor-[DATE].md`. It surfaces the source list, cleanup changes, supported claims, `[TO DO]` gaps, and decisions that require a human boss.
